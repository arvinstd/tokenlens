<template>
  <div style="padding: 0 40px">
    ...
    <div style="margin-top: 32px; border: 1px solid #E4E4E7; border-radius: 14px; overflow: hidden; max-width: 560px">
      ...
      <div v-if="connection.connected" style="padding: 20px">
        ...
        <div style="display: flex; align-items: center; gap: 8px; padding: 12px 16px; background: #F0FDF4; border: 1px solid #BBF7D0; border-radius: 10px; margin-bottom: 16px">
          ...
          <div style="flex: 1">
            ...
            <div style="font-size: 12px; color: #71717A; margin-top: 1px">
              ...
              <template v-if="connection.lastSynced"> &middot; Last sync {{ formatDate(connection.lastSynced) }}</template>
            </div>
          </div>
        </div>
        ...
        <div v-if="syncResult" style="margin-top: 12px; padding: 10px 14px; background: #F0FDF4; border: 1px solid #BBF7D0; border-radius: 8px; font-size: 12px; color: #166534">
          Synced {{ syncResult.tokenCount }} tokens &middot; {{ syncResult.componentCount }} components across {{ Object.keys(syncResult.categories).length }} categories
        </div>
      </div>
      ...
      <div v-else style="padding: 20px">
        ...
        <div style="margin-bottom: 16px">
          ...
          <input
            v-model="fileUrl"
            type="text"
            placeholder="https://figma.com/design/abc123/My-Design-System"
            class="input-field"
          />
        </div>

        <!-- Error -->
        <div v-if="error" style="margin-bottom: 12px; padding: 10px 14px; background: #FEF2F2; border: 1px solid #FECACA; border-radius: 8px; font-size: 12px; color: #DC2626">
          {{ error }}
        </div>
    ...
    <div style="margin-top: 16px; border: 1px solid #E4E4E7; border-radius: 14px; overflow: hidden; max-width: 560px">
        ...
        <div style="display: flex; align-items: center; gap: 8px; padding: 12px 16px; background: #F0FDF4; border: 1px solid #BBF7D0; border-radius: 10px; margin-bottom: 16px">
          ...
          <div style="flex: 1">
            ...
            <div style="font-size: 12px; color: #71717A; margin-top: 1px">
              ...
              <template v-if="ghConnection.lastSynced"> &middot; Last sync {{ formatDate(ghConnection.lastSynced) }}</template>
            </div>
          </div>
        </div>
      ...
      <div v-else style="padding: 20px">
        ...
        <!-- Error -->
        <div v-if="ghError" style="margin-bottom: 12px; padding: 10px 14px; background: #FEF2F2; border: 1px solid #FECACA; border-radius: 8px; font-size: 12px; color: #DC2626">
          {{ ghError }}
        </div>
        ...
        <button
          class="btn-primary"
          :disabled="!ghPat || !ghRepoUrl || ghLoading"
    ...
    <!-- Diff Section -->
    <div v-if="connection.connected && ghConnection.connected" style="margin-top: 24px; max-width: 560px">
      <div style="border: 1px solid #E4E4E7; border-radius: 14px; overflow: hidden">
        <div style="padding: 20px; border-bottom: 1px solid #F0F0F1; display: flex; align-items: center; gap: 12px">
          <div style="width: 36px; height: 36px; border-radius: 10px; background: #FFF7ED; display: flex; align-items: center; justify-content: center; flex-shrink: 0">
            <GitCompareArrows :size="18" color="#EA580C" />
          </div>
          <div style="flex: 1">
            <div style="font-size: 14px; font-weight: 600; color: #09090B">Token Diff</div>
            <div style="font-size: 12px; color: #71717A">Compare Figma tokens vs Code tokens</div>
          </div>
        </div>
        <div style="padding: 20px">
          ...
          <div v-if="diffSummary" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 16px">
            ...
            <div style="text-align: center; padding: 10px; background: #FEF2F2; border-radius: 8px">
              ...
              <div style="font-size: 11px; color: #71717A; margin-top: 2px">Missing</div>
            </div>
            <div style="text-align: center; padding: 10px; background: #EFF6FF; border-radius: 8px">
              ...
              <div style="font-size: 11px; color: #71717A; margin-top: 2px">Code only</div>
            </div>
          </div>
